...
from app.services.onboarding_wizard import (
    ...
    generate_notification, STEPS, FALLBACK_SESSION_THRESHOLD,
...
# ---------------------------------------------------------------------------
@router.get("/steps")
def list_steps(current_user: dict = Depends(get_current_user)):
    """List all wizard steps with descriptions."""
    return {
        "total_steps": len(STEPS),
        "steps": [
            {
                ...
                "description": "Enter NID number, DOB and provide fingerprint biometric",
                "required_fields": ["nid_number", "dob", "fingerprint_b64"],
            },
            {
                "step_number": 2,
                "name":        "PERSONAL_INFO",
                "description": "OCR-populated personal info form with nominee fields",
                "required_fields": ["full_name", "mobile"],
            },
            {
                "step_number": 3,
                "name":        "PHOTOGRAPH",
                "description": "Capture or upload customer photograph",
                "required_fields": ["photo_b64"],
            },
            {
                "step_number": 4,
                "name":        "SIGNATURE",
                "description": "Wet/electronic/digital signature or PIN for low-risk",
                "required_fields": ["signature_type"],
                "allowed_types_low_risk":  SIGNATURE_TYPES_LOW_RISK,
                "allowed_types_high_risk": SIGNATURE_TYPES_HIGH_RISK,
            },
            {
                "step_number": 5,
                "name":        "NOTIFICATION",
                "description": "Account opening notification via registered SIM and email",
