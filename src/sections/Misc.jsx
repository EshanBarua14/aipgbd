  ...
  return (
    <div style={{ borderTop:'1px solid var(--border-dim)', borderBottom:'1px solid var(--border-dim)', padding:'0.8rem 0', overflow:'hidden', zIndex:2, position:'relative' }}>
      <div style={{ display:'flex', animation:'ticker 36s linear infinite', whiteSpace:'nowrap' }}>
        {all.map((t, i) => (
          <span key={i} style={{ display:'inline-flex', alignItems:'center', gap:'1rem', padding:'0 2rem', fontFamily:'var(--font-mono)', fontSize:'0.6rem', letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--text-muted)', flexShrink:0 }}>
            <span style={{ width:4, height:4, background:'var(--gold)', borderRadius:'50%', display:'inline-block', flexShrink:0 }} />{t}
          </span>
        ))}
      </div>
    </div>
          ...
          {PROCESS.map((p, i) => {
            ...
            return (
              <div key={p.phase} ref={r}
                style={{
                  padding:'2.4rem 2rem', background:'var(--bg)', position:'relative', overflow:'hidden',
              ...
              >
                ...
                <div style={{ fontFamily:'var(--font-mono)', fontSize:'0.58rem', letterSpacing:'0.2em', color:'var(--gold)', marginBottom:'0.35rem' }}>Phase {p.phase}</div>
                <div style={{ fontFamily:'var(--font-mono)', fontSize:'0.58rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'var(--text-muted)', marginBottom:'1.1rem' }}>{p.tool}</div>
                ...
                <div style={{ position:'absolute', bottom:'1.4rem', right:'1.4rem', fontFamily:'var(--font-display)', fontSize:'2.8rem', fontWeight:800, color:'rgba(201,168,76,0.045)', lineHeight:1, userSelect:'none' }}>{p.phase}</div>
              </div>
            );
          })}
        </div>
    ...
    <section id="niches" style={{ background:'var(--bg-1)', padding:'7rem 0', borderTop:'1px solid var(--border-dim)' }}>
      <div className="container">
        <div className={`reveal ${v ? 'visible' : ''}`} ref={ref}>
          ...
          <h2 className="section-title">We Solve<br /><span style={{ color:'var(--gold)' }}>Specific Problems.</span></h2>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:'1px', background:'var(--border-dim)', marginTop:'3.5rem', border:'1px solid var(--border-dim)' }}>
          {NICHES.map((n, i) => {
            ...
            return (
              ...
              >
                <div style={{ fontSize:'1.4rem', marginBottom:'1rem' }}>{n.icon}</div>
                <h3 style={{ fontFamily:'var(--font-display)', fontSize:'1.05rem', fontWeight:700, marginBottom:'0.5rem' }}>{n.title}</h3>
                <p style={{ fontSize:'0.8rem', color:'var(--text-muted)', lineHeight:1.8, marginBottom:'1rem' }}>{n.pain}</p>
                <div style={{ fontFamily:'var(--font-mono)', fontSize:'0.65rem', letterSpacing:'0.06em', color:'var(--gold)', display:'flex', alignItems:'center', gap:'0.4rem' }}>
                  <span>→</span>{n.roi}
                </div>
              </div>
            );
      ...
      <div className="container">
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1.2rem', marginBottom:'1.2rem' }}>
          ...
          <div style={{ display:'flex', gap:'2rem' }}>
            {[{ label:'Facebook', href:SITE.facebook }, { label:'YouTube', href:SITE.youtube }].map(l => (
              <a key={l.label} href={l.href} target="_blank" rel="noreferrer"
                style={{ fontFamily:'var(--font-mono)', fontSize:'0.62rem', letterSpacing:'0.1em', textTransform:'uppercase', color:'var(--text-muted)', transition:'color 0.25s' }}
                onMouseEnter={e => e.target.style.color = 'var(--gold)'}
                onMouseLeave={e => e.target.style.color = 'var(--text-muted)'}>{l.label}</a>
            ))}
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:'0.6rem', color:'var(--text-muted)' }}>© 2026 AI Playground BD</span>
        </div>
        {/* System status */}
        <div style={{ display:'flex', alignItems:'center', gap:'1rem', flexWrap:'wrap', padding:'0.65rem 1.1rem', border:'1px solid var(--border-dim)', fontFamily:'var(--font-mono)', fontSize:'0.58rem', letterSpacing:'0.13em', color:'var(--text-muted)' }}>
